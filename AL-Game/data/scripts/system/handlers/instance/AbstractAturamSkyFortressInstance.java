...
/**
 * Holds everything the Aturam Sky Fortress instance and its event variant run identically.
 *
 * The two handlers shared 27 byte-identical methods. Every one of them
 * lives here; each subclass keeps only what it does differently.
 *
 * @author Oraion
 */
public abstract class AbstractAturamSkyFortressInstance extends GeneralInstanceHandler {

	protected int energyGenerators;
	protected int balaurSpyCrystal;
	protected int drakanChiefOfStaff;
	protected int drakanPettyOfficer;
	...
	protected final List<Future<?>> aturamSkyFortressTask = new ArrayList<>();
	...
	public void onDie(Npc npc) {
		...
		switch (npc.getObjectTemplate().getTemplateId()) {
			...
			case 218577: //Marabata Watchman.
				/**
				 * Ã¢â€“Â  At home in the Aturam Sky Fortress Command Center
	...
	protected void rushWalk(final Npc npc) {
	...
	protected void startMarbataWalkerEvent() {
	...
	protected void startOfficerWalkerEvent() {
	...
	protected void removeItems(Player player) {
	...
	protected void addHarikenItems(Player player) {
	...
	protected void stopInstanceTask() {
        for (Future<?> n : aturamSkyFortressTask) {
            if (n != null) {
                n.cancel(true);
            }
        }
