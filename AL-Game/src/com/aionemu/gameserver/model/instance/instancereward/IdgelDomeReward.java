...
public class IdgelDomeReward extends InstanceReward<IdgelDomePlayerReward> {
	...
	public List<IdgelDomePlayerReward> sortPoints() {
		return getInstanceRewards().stream()
				.sorted(Comparator.comparingInt(IdgelDomePlayerReward::getScorePoints).reversed())
	...
	public boolean hasCapPoints() {
		return getInstanceRewards().stream().mapToInt(IdgelDomePlayerReward::getPoints).max().orElse(0) >= capPoints;
	}
}