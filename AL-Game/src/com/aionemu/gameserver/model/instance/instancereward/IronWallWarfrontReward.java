...
public class IronWallWarfrontReward extends InstanceReward<IronWallWarfrontPlayerReward> {
	...
	public List<IronWallWarfrontPlayerReward> sortPoints() {
		return getInstanceRewards().stream()
				.sorted(Comparator.comparingInt(IronWallWarfrontPlayerReward::getScorePoints).reversed())
	...
	public boolean hasCapPoints() {
		return getInstanceRewards().stream().mapToInt(IronWallWarfrontPlayerReward::getPoints).max().orElse(0) >= capPoints;
	}
}